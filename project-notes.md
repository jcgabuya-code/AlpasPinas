...
**Status (2026-06-26): 🎨 HOME PAGE critique + polish (Impeccable) — committed.** Ran `critique`→`polish` (+ targeted `layout`/`colorize`) on the home page; one focused commit landed on `release/alpaspinas-v2` with the 6 pure-design files (Hero, Features, Team, Contact, SectionHeader, index.css). See the 2026-06-26 log. ⚠️ Open: **Contact form backend still unwired** (clearly-marked `submitApplication()` INTEGRATION POINT in `Contact.tsx`); light-mode accent-text contrast is a pre-existing AA gap; the `FeaturedGear.tsx` tweak stays in the uncommitted merch pile (it imports merch code) to land with the merch commit grouping below.

**Status (2026-06-25): 🛍️ MERCH SHOP built (steps 1–4 + admin + delivery + order tracking).** A full reserve-only team store. See the 2026-06-25 session log for the complete picture. Reference doc: `merch-feature-plan.md`. **Still entirely uncommitted** (commit grouping in that log).

**⚠️ DEPLOY GATE — run on JC's Mac before the shop works live:**
1. `node scripts/deploy-migration.mjs` — applies the LATEST merch migration. **Two merch migrations exist now:** `20260625000000_merch.sql` (products + merch_orders, already deployed 2026-06-25) and `20260625100000_merch_delivery.sql` (delivery fields — **NOT yet confirmed deployed**). The script applies the newest by default; pass a name to target a specific one. Re-run it for the delivery migration if not done.
2. `node scripts/seed-products.mjs` — upserts the 2 sample products (Drifit Shirt featured + promo, Racerback Tank). Already run once 2026-06-25 with the real image set.
3. `npm run build` — **never verified in the Cowork sandbox** (missing Linux ARM64 rolldown binding); confirm it bundles clean on the Mac before deploying.
4. Everything is **uncommitted** — commit grouping is in the 2026-06-25 log.

**Merch open follow-ups (none blocking):**
- Guest order tracking: `/orders` only shows orders placed while LOGGED IN (they get `user_id`). Guest checkouts aren't tied to an account. To let guests track, add an emailed order-reference lookup.
- Order-notification email to the team on new reservations (could reuse the Apps Script Gmail mailer).
- Whole-app 44px touch-target pass: the main site nav icon buttons (2.25rem) + admin mobile hamburger (34px) are still under 44px — deliberately left consistent, not yet bumped. Shop + new admin merch screens ARE at 44px.
- `shirt-1.png`/`racerback-tank.png` in `public/shop/` are now unused (replaced by `shirt-model1*`/`tank-model1*`); safe to delete.
...
### 2026-06-26 — Home page critique + polish (Impeccable)
- Ran Impeccable `critique` → `polish` (plus targeted `layout` + `colorize`) on the home page. Critique scored **31/40** (snapshot in `.impeccable/critique/`, still untracked). Detector clean on all home files; `tsc --noEmit` clean throughout.
- **Hero (`Hero.tsx`):** the two CTAs now share **one row at every breakpoint** — trimmed labels ("Book a Session →", "Watch Race"), smaller font/padding, `flex:1` on mobile / natural width on desktop, ≥44px tall (aria-labels keep the full descriptive text). Removed the redundant soundwave **cadence meter** (kept the SPEED·SYNC·STRENGTH beat row) to de-densify the fold + dropped the now-unused `cadenceAccentUri` import. **Stat numerals** now use the brand accent (theme-aware: `primaryLight` dark / `primaryDark` light — both verified AA) so the strip reads intentional, not placeholder.
- **Gradient-text ban fixed (`Features.tsx`):** "BUILT FOR THE WATER" used `background-clip:text` (an absolute Impeccable ban *and* a PRODUCT.md anti-reference) → re-voiced to **"WHY WE PADDLE"** with a solid accent, which also kills the "THE WATER" echo with the hero. Added the missing `borderTop` hairline so it matches Team/Contact.
- **Eyebrow thinning:** made `SectionHeader.eyebrow` **optional**; dropped it from FeaturedGear + Team (they have orienting trailing links), kept it on Features + Contact. Stops the eyebrow-on-every-section AI tell (DESIGN.md explicitly warns against it).
- **Contact form (`Contact.tsx`):** replaced the dead `submitted` boolean with an `idle → submitting → success | error` state machine. Submit button shows a spinner + "Claiming your seat…" (new `spin` keyframe in `index.css`); added a `role="alert"` error banner with an on-system `AlertIcon` + mailto fallback; success block got `role="status"`. **⚠️ Backend still NOT wired** — there's a clearly-marked `submitApplication()` **INTEGRATION POINT** (swap for a Supabase `applications` insert / email relay; resolve on success, throw on failure and the states stay wired). The 900ms stub only exists to make the loading state visible.
- **Layout:** FeaturedGear desktop grid `auto-fill` → `auto-fit` (was reserving dead phantom tracks → undersized, left-clumped tiles on wide screens). Team crew grid goes **2-up on phones** via `minmax(min(47%, 200px), 1fr)` (desktop column count unchanged).
- **Committed (this session):** `Hero.tsx Features.tsx Team.tsx Contact.tsx SectionHeader.tsx index.css` + this notes file, as one focused commit on `release/alpaspinas-v2`. These 6 files import no merch code, so they're self-contained.
- **⚠️ Left in the working tree on purpose:** my `FeaturedGear.tsx` tweak (eyebrow drop + `auto-fit` grid) — it imports `ProductCard`/`merch.ts` which are still untracked, so committing it alone would break the build. It rides along with the merch commit grouping (2026-06-25 log) instead. The whole merch pile + Impeccable tooling (`PRODUCT.md DESIGN.md .impeccable/ .claude/skills/` etc.) remain uncommitted as before.
- **Open follow-ups:** (1) **wire the Contact form backend** (the INTEGRATION POINT); (2) **light-mode accent-text contrast** is a pre-existing systemic AA gap — trailing links + headline accent words use `c.primary` (~2.3:1 on the light bg); fix by routing accent *text* through a contrast-tuned token (e.g. `primaryDark` in light) with dark untouched; (3) verify the hero stats (12+ / 5 YRS / #3) are real numbers, not placeholders.

### 2026-06-25 — Merchandise shop (catalog, reserve checkout, promo, admin, delivery, order tracking)
- **Brief:** merch page; many items but featured/promo to grab attention; visitor notices the promo at first glance. Decisions: **reserve-only checkout** (no online payment — collected manually), **Supabase catalog**, **promo = announcement bar + featured section**, **guest checkout allowed**, register = **brand**, WCAG **AA**, **mobile-first**.
- **Design tooling installed:** `frontend-design` + `ui-ux-pro-max` (anthropic skills) and **Impeccable** (`npx impeccable install` → `.claude/skills/impeccable/`). Wrote `PRODUCT.md` (register=brand) + `DESIGN.md` (captured the real token system + the mobile-first approach) via Impeccable's init flow. Slash commands need a Cowork reload to register, but the skill was driven by reading its references directly. Detector (`node .claude/skills/impeccable/scripts/detect.mjs --json <files>`) run on every shop file → all clean.
- **DB (`supabase/migrations/`):** `20260625000000_merch.sql` — `products` (RLS: public reads active, admin writes) + `merch_orders` (RLS: anyone inserts pending, owner+admin read, admin updates). `20260625100000_merch_delivery.sql` — added `delivery_method` (pickup/delivery), `delivery_address`, `estimated_delivery` + a cross-field CHECK (delivery requires an address). Idempotent, follows the `training_signups` pattern. Deployed via the Management-API-over-HTTPS workaround (ports 5432/6543 blocked) — see new `scripts/deploy-migration.mjs` (generic, keychain token, records version) + `scripts/seed-products.mjs` (upsert on slug).
- **Data layer (`src/utils/merch.ts`):** Supabase-backed with local fallback. Products: `fetchProducts`/`fetchProduct`/`fetchFeaturedProducts`/`fetchAllProducts` (admin) + `createProduct`/`updateProduct`/`setProductFlag`/`deleteProduct`/`slugify`. Orders: `submitOrder` (bare anon insert), `fetchOrders` (admin board, own-or-all), `fetchMyOrders` (strictly own — filters by auth uid), `updateOrderStatus`, `setOrderEstimatedDelivery`. Helpers: `effectivePrice`, `orderTotal`, `formatPrice` (RM/₱), `inStock`.
- **Cart:** client-side `CartContext` (localStorage `alpas-cart-v1`, cross-tab sync, snapshots name+price per line) wired in `App.tsx`. Nav has a Shop link + live cart-count badge (sun accent), desktop + mobile.
- **Shop UI (all lazy routes):** `/shop` (`Shop.tsx`) — featured **spotlight** (distinct `FeaturedSpotlight`, NOT a duplicate card — fixed a redundancy where the featured item showed twice; it's now excluded from the grid) + "ALL GEAR" filtered grid. `/shop/:slug` (`ProductDetail.tsx`) — gallery, size/qty, add-to-cart, promo/struck price, sold-out/not-found. `/cart` (`Cart.tsx`) — line items + reserve checkout (name/email/phone/note, prefilled if logged in), **delivery method toggle (pickup/delivery) + conditional required address**, success state. Shared `ProductCard.tsx`.
- **Promo (home):** `PromoBar.tsx` (site-wide, above sticky nav, full-bleed sun accent, dismissible per-promo via localStorage `alpas-promo-dismissed`, no animation) + `FeaturedGear.tsx` band on `Home.tsx` after hero/marquee. To re-show a dismissed bar: `localStorage.removeItem('alpas-promo-dismissed')`.
- **Admin (`/admin`):** `AdminProducts.tsx` (list incl. hidden; create/edit form; toggle Active/Featured; delete-with-confirm) + `AdminOrders.tsx` (status board pending→confirmed→paid→fulfilled/cancelled; shows delivery method/address; **editable estimated-delivery date picker**). Wired into `Admin.tsx` SECTIONS + switch + `AdminSection` type.
- **Customer order tracking:** `/orders` (`MyOrders.tsx`, login-gated via RequireAuth) — Active orders with a status tracker (Reserved→Confirmed→Paid→Fulfilled) + delivery info + ETA; History (fulfilled/cancelled). Linked from the desktop account dropdown + mobile drawer. Uses `fetchMyOrders` so admins see only their own here.
- **Mobile-first:** 2-up overflow-safe grids (`minmax(min(100%, …), 1fr)`), ≥44px touch targets across shop + merch admin, 16px inputs (no iOS zoom). `DESIGN.md` records the approach.
- **Verification:** `npx tsc -b` clean throughout (only the pre-existing `useInView.ts` error remains — unrelated). Vite build NOT runnable in sandbox.
- **Commit grouping (uncommitted):** docs (`PRODUCT.md DESIGN.md merch-feature-plan.md`); backend (`supabase/migrations/2026062510*.sql 20260625000000_merch.sql scripts/deploy-migration.mjs scripts/seed-products.mjs src/utils/merch.ts`); shop UI (`src/context/CartContext.tsx` + the 5 components + 4 pages: Shop/ProductDetail/Cart/MyOrders); admin + wiring (`AdminProducts AdminOrders Admin.tsx App.tsx Navigation.tsx Footer.tsx Layout.tsx Home.tsx`); images (`public/shop/`).
- **Where we left off:** feature-complete pending the DEPLOY GATE above (run delivery migration + build on Mac + commit). Open follow-ups in Resume-here.
