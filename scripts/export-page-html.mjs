...
// Usage: node scripts/export-page-html.mjs <route> <name> [themes...]
// Example: node scripts/export-page-html.mjs /training training light dark
const [, , routeArg, nameArg, ...themeArgs] = process.argv;
const ROUTE = routeArg || '/';
const NAME = nameArg || 'page';
...
async function exportTheme(browser, theme) {
  ...
  const html = await page.evaluate(async () => {
    ...
    // (populates currentSrc for sliders, galleries, etc.).
  ...
  const outPath = path.join(OUT_DIR, `${NAME}-${theme}.html`);
...
async function main() {
  ...
  for (const theme of THEMES) {
    await exportTheme(browser, theme);
  }
  await browser.close();
}
